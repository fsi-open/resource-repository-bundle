...
class FSiCKEditorTypeSpec extends ObjectBehavior
{
    public function let()
    {
        $this->beConstructedWith('resource_group.resource_fsi_ckeditor');
    }

    function it_is_initializable()
    {
        $this->shouldHaveType('FSi\Bundle\ResourceRepositoryBundle\Repository\Resource\Type\FSiCKEditorType');
    }
    ...
    {
        $this->getName()->shouldReturn('resource_group.resource_fsi_ckeditor');
    }

    function it_return_form_builder(FormFactory $factory, FormBuilder $form)
    {
        $factory->createNamedBuilder('textValue', 'fsi_ckeditor', null, array(
            'label' => false,
            'required' => false,
